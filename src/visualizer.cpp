...
namespace Model
{
    ...
    {
        string label = s.name;
        for (auto i = s.defferedEvents.begin(), e = s.defferedEvents.end(); i != e; ++i)
            label.append("<br />").append(*i).append(" / defer");
        for (auto i = s.inStateEvents.begin(), e = s.inStateEvents.end(); i != e; ++i)
    ...
    public:
        ...
        void write_as_dot_file(string fn)
        {
            ...
            for (auto i = begin(), e = end(); i != e; i++)
                f << i->second;
            for (auto t = transitions.begin(), e = transitions.end(); t != e; ++t)
                f << **t;
            f << indent_dec << "}\n";
        }
    };
...
{
public:
    bool isDerivedFrom(const char* baseStr, const CXXBaseSpecifier** Base = 0) const
    {
...
public:
    ...
    bool VisitMemberExpr(MemberExpr* E)
    {
        ...
        if (E->hasExplicitTemplateArgs())
        {
            ...
            CXXRecordDecl* Event = EventType->getAsCXXRecordDecl();
            auto T = new Model::Transition(SrcState->getName(), DstState->getName(), Event->getName());
...
class Visitor : public RecursiveASTVisitor<Visitor>
{
    ...
    std::vector<bool> reactMethodInReactions;
    // Indicates whether i-th react method is referenced from typedef reactions.
...
public:
    ...
    void HandleReaction(const Type* T, const SourceLocation Loc, CXXRecordDecl* SrcState)
    {
        ...
        else if (const TemplateSpecializationType* TST = dyn_cast<TemplateSpecializationType>(T))
        {
            ...
            {
                const Type* EventType = TST->template_arguments()[0].getAsType().getTypePtr();
                const Type* DstStateType = TST->template_arguments()[1].getAsType().getTypePtr();
                CXXRecordDecl* Event = EventType->getAsCXXRecordDecl();
                CXXRecordDecl* DstState = DstStateType->getAsCXXRecordDecl();
                unusedEvents.remove_if(eventHasName(Event->getNameAsString()));

                auto T = new Model::Transition(SrcState->getName().str(), DstState->getName().str(),
                                               Event->getName().str());
                model.transitions.push_back(T);
            }
            ...
            {
                for (auto Arg = TST->template_arguments().begin(), End = TST->template_arguments().end(); Arg != End; ++
                     Arg)
    ...
    TemplateArgumentLoc getTemplateArgLoc(const TypeLoc& T, unsigned ArgNum, bool ignore)
    {
        ...
        else if (const TemplateSpecializationTypeLoc TST = T.getAs<TemplateSpecializationTypeLoc>())
        {
            if (TST.getNumArgs() >= ArgNum + 1)
            {
                ...
                Diag(TST.getBeginLoc(), diag_warning) << TST.getType()->getTypeClassName() << "has not enough arguments"
                    << TST.getSourceRange();
        }
        else
            Diag(T.getBeginLoc(), diag_warning) << T.getType()->getTypeClassName() <<
                                            ...
                                            bool ignore = false)
    {
        ...
        case TemplateArgument::Null:
            ...
            break;
        default:
            Diag(Loc.getSourceRange().getBegin(), diag_warning) << Loc.getArgument().getKind() << "unsupported kind" <<
                Loc.getSourceRange();
        }
        return 0;
    }
    ...
    void handleSimpleState(CXXRecordDecl* RecordDecl, const CXXBaseSpecifier* Base)
    {
        ...
        if (Context)
        {
            ...
            if (!c)
            {
                auto s = new Model::State(Context->getName());
            ...
            static_cast<MyCXXRecordDecl*>(getTemplateArgDeclOfBase(Base, 2, Loc, true)))
        {
                ...
                InnerInitialState->isDerivedFrom("boost::statechart::state_machine"))
            {
                ...
                Diag(Loc.getTypeSourceInfo()->getTypeLoc().getBeginLoc(), diag_warning)
                    << InnerInitialState->getName() << " as inner initial state is not supported" << Loc.
                    getSourceRange();
        }
    ...
    bool VisitCXXRecordDecl(CXXRecordDecl* Declaration)
    {
        ...
        auto RecordDecl = static_cast<MyCXXRecordDecl*>(Declaration);
    ...
    {
        for (auto it = unusedEvents.begin(); it != unusedEvents.end(); it++)
    ...
    _In_ LPVOID lpvReserved)
{
    ...
    OutputDebugStringA("DllMain process was init");


    FrontendPluginRegistry::Add<VisualizeStatechartAction>* X = nullptr;
    switch (fdwReason)
    {
    case DLL_PROCESS_ATTACH:
        OutputDebugStringA("DllMain process attached!");
    // Initialize once for each new process.
    // Return FALSE to fail DLL load.
        __debugbreak();
        X = new FrontendPluginRegistry::Add<VisualizeStatechartAction>("visualize_statechart", "visualize statechart");
    ...
    case DLL_THREAD_ATTACH:
        ...
        {
            OutputDebugStringA("DllMain Thread Attached");
            __debugbreak();
            X = new FrontendPluginRegistry::Add<VisualizeStatechartAction>(
            "visualize_statechart", "visualize statechart");
