...
namespace Model
{
	...
	{
		list<string> initialInnerStates;
	...
	public:
		...
		void addInitialInnerState(string name) { initialInnerStates.push_back(name); }
	...
	ostream &operator<<(ostream &os, const State &s)
	{
		...
		if (s.size())
		{
			if (s.initialInnerStates.size() > 0)
			{
				// lhead attribute is to make sure to link to one of the initial states, but linking to the cluster instead.
				os << indent << s.name << " -> " << *s.initialInnerStates.begin() << " [style = dashed, lhead=cluster_" << s.name << "]\n";
			}
			os << indent << "subgraph cluster_" << s.name << " {\n"
			   << indent_inc;
			os << indent << "label = \"" << s.name << "\"\n";
			for (const auto &state : s.initialInnerStates)
			{
				os << indent << state << " [peripheries=2]\n";
			}
...
public:
	...
	void handleSimpleState(CXXRecordDecl *RecordDecl, const CXXBaseSpecifier *Base)
	{
		...
		if (Context)
		{
			std::string contextName(Context->getNameAsString());
			if (contextName == "orthogonal")
			{
				// Need to check that it's a simple state.
				// full qualified name of orthogonal would be something like:
				//
				//  "boost::statechart::simple_state<ParentClass, ParentOfParentClass, InitialInnerStatesClassOrClasses, boost::statechart::has_no_history>::orthogonal"
				//
				// We will register as child of ParentClass
				//
				ClassTemplateSpecializationDecl *specDecl = llvm::dyn_cast<ClassTemplateSpecializationDecl>(Context);
				ClassTemplateDecl *ctSpecDecl = specDecl->getSpecializedTemplate();

				CXXRecordDecl *tplContext = ctSpecDecl->getTemplatedDecl();
				const DeclContext *Ctx = tplContext->getDeclContext();
				const NamedDecl *ND1 = llvm::dyn_cast<NamedDecl>(Ctx);

				const ClassTemplateSpecializationDecl *specDecl2 = llvm::dyn_cast<ClassTemplateSpecializationDecl>(ND1);
				const TemplateArgumentList &templateArgs = specDecl2->getTemplateArgs();
				// We have the template argument list to simple_state, get the first argument
				if (templateArgs[0].getKind() == TemplateArgument::Type)
				{
					QualType qualType = templateArgs[0].getAsType();
					if (const RecordType *recordType = dyn_cast<RecordType>(qualType.getTypePtr()))
					{
						...
						contextName = recordDecl->getNameAsString();
					}
					// Not sure if we need this here. This is in the case the QualType object is a typedef for a struct type
					if (const TypedefType *typedefType = dyn_cast<TypedefType>(qualType.getTypePtr()))
					{
						const TypedefNameDecl *typedefDecl = typedefType->getDecl();
						if (const RecordType *recordType = dyn_cast<RecordType>(typedefDecl->getUnderlyingType().getTypePtr()))
						{
							const clang::RecordDecl *recordDecl = recordType->getDecl();
							contextName = recordDecl->getNameAsString();
						}
			...
			Model::Context *c = model.findContext(contextName);
			if (!c)
			{
				Model::State *s = new Model::State(contextName);
				...
				static_cast<MyCXXRecordDecl *>(getTemplateArgDeclOfBase(Base, 2, Loc, true)))
		{
			...
			{
				state->addInitialInnerState(InnerInitialState->getName().str());
			}
			else if (!InnerInitialState->getQualifiedNameAsString().compare("boost::mpl::list"))
			{
				const ClassTemplateSpecializationDecl *specDecl2 = llvm::dyn_cast<ClassTemplateSpecializationDecl>(InnerInitialState);
				const TemplateArgumentList &templateArgs = specDecl2->getTemplateArgs();
				for (unsigned int i = 0; i < templateArgs.size(); ++i)
				{
					if (templateArgs[i].getKind() == TemplateArgument::Type)
					{
						QualType qualType = templateArgs[i].getAsType();
						std::string typeName(qualType.getAsString());
						
						// skip extra template arguments
						if (typeName == "struct mpl_::na")
							continue;

						if (const RecordType *recordType = dyn_cast<RecordType>(qualType.getTypePtr()))
						{
							const clang::RecordDecl *recordDecl = recordType->getDecl();
							state->addInitialInnerState(recordDecl->getNameAsString());
						}
					}
				}
			}
...
void
handleStateMachine(CXXRecordDecl *RecordDecl, const CXXBaseSpecifier *Base)
{
